...
launch_process(process* p)
{
    ...
    free(path);
    fprintf(stdout, "This shell only supports built-ins. Please take a look at help command (>>> ?).\n");
    fprintf(stdout, "این پوسته فقط از توکارها پشتیبانی می کند. لطفاً به دستور کمک (? <<<) نگاهی بیندازید.\n");
}
